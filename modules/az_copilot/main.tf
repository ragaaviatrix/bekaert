...
resource "azurerm_network_interface" "aviatrix_copilot_nic" {
  ...
  ip_configuration {
    ...
    subnet_id                     = var.use_existing_vnet == false ? azurerm_subnet.aviatrix_copilot_subnet[0].id : data.azurerm_subnet.ctrl_sub.id
...
resource "azurerm_managed_disk" "vol-one" {
  name                 = "vol1"
  location             = var.location
...
resource "azurerm_linux_virtual_machine" "aviatrix_copilot_vm" {
  ...
  # depends_on = [
  #   azurerm_marketplace_agreement.aviatrix-controller
  # ]
}
